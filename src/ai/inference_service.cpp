...
InferenceService::InferenceService(std::unique_ptr<IObjectDetector> detector,
                                   std::shared_ptr<Queue> input, InferenceConfig config,
                                   std::shared_ptr<spdlog::logger> logger)
    : InferenceService(single(std::move(detector)), std::move(input), std::move(config),
                       std::move(logger)) {}

InferenceService::InferenceService(std::vector<std::unique_ptr<IObjectDetector>> detectors,
                                   std::shared_ptr<Queue> input, InferenceConfig config,
                                   std::shared_ptr<spdlog::logger> logger)
    : detectors_(std::move(detectors)), input_(std::move(input)), config_(std::move(config)),
      writer_(config_.result_path, config_.event_socket), logger_(std::move(logger)) {
    if (!input_ || !config_.fps || config_.fps > 60 || config_.result_path.empty() ||
        ...
        std::any_of(detectors_.begin(), detectors_.end(), [](const auto& d) {
            return !d;
        }))
...
bool InferenceService::start() {
    if (running_)
        return true;
    if (worker_.joinable())
        throw std::logic_error("join AI before restarting");
    stop_ = false;
    running_ = true;
    try {
...
void InferenceService::request_stop() noexcept {
    stop_ = true;
}

void InferenceService::join() noexcept {
    if (worker_.joinable())
        worker_.join();
    running_ = false;
}

bool InferenceService::running() const noexcept {
    return running_;
}
...
core::HealthSnapshot InferenceService::health() const {
    ...
    return {running_ ? (ready == config_.workers && detail.empty() ? core::ServiceState::running
                                                                   : core::ServiceState::degraded)
                     : core::ServiceState::stopped,
            detail};
}
...
void InferenceService::run() noexcept {
    ...
    try {
        ...
        core::ThreadPool pool(detectors_.size(), [this](std::size_t i) {
            detectors_[i]->close();
        });
        ...
        std::size_t cursor = 0;
        if (logger_)
            logger_->info("[ai] workers={}, core_policy={}, total fps={}", config_.workers,
                          config_.core_policy, config_.fps);
        while (!stop_) {
            auto now = Clock::now();
            ...
            if (auto frame = input_->pop_for(std::chrono::milliseconds(10))) {
                ...
                if (frame->source_generation == *generation && now - frame->timestamp <= max_age &&
                    (!pending || frame->sequence > pending->sequence))
            ...
            for (std::size_t i = 0; i < workers.size(); ++i) {
                ...
                if (worker.job && worker.job->wait_for(std::chrono::milliseconds(0)) ==
                                      std::future_status::ready) {
                    try {
                        auto result = worker.job->get();
                        worker.job.reset();
                        if (!result)
                            worker.ready = true;
                        else if (generation && result->source_generation == *generation &&
                                 (!last_sequence || result->sequence > *last_sequence) &&
                                 now - result->source_time <= max_age) {
                        ...
                        worker.job.reset();
                        worker.ready = false;
                        worker.retry = now + std::chrono::seconds(2);
                        try {
                            throw;
                        } catch (const std::exception& e) {
                            try {
                                if (logger_)
                                    logger_->warn("[ai] worker {}: {}; retry in 2s", i, e.what());
                            } catch (...) {
                            }
                        } catch (...) {
                            try {
                                if (logger_)
                                    logger_->warn("[ai] worker {} failed; retry in 2s", i);
                            } catch (...) {
                            }
                        }
                    }
                ...
                if (!worker.ready && !worker.job && now >= worker.retry) {
                    worker.job = pool.try_submit(i, [this](std::size_t index) -> Result {
                        try {
                            detectors_[index]->open();
                            return std::nullopt;
                        } catch (...) {
                            detectors_[index]->close();
                            throw;
                        }
                    });
                }
            }
            ready_workers_ = static_cast<unsigned>(
                std::count_if(workers.begin(), workers.end(), [](const auto& w) {
                    return w.ready;
                }));
            if (pending && now - pending->timestamp > max_age)
                pending.reset();
            if (pending && now >= next) {
                for (std::size_t offset = 0; offset < workers.size(); ++offset) {
                    const auto i = (cursor + offset) % workers.size();
                    auto& worker = workers[i];
                    if (!worker.ready || worker.job)
                        continue;
                    auto job =
                        pool.try_submit(i, [this, frame = *pending](std::size_t index) -> Result {
                            try {
                                ...
                                result.worker_index = static_cast<unsigned>(index);
                                return result;
                            } catch (...) {
                                detectors_[index]->close();
                                throw;
            ...
            if (now >= heartbeat) {
                if (!ready_workers_)
                    publish("unavailable");
                else if (now - last_result > std::chrono::seconds(1))
                    publish("waiting");
        ...
        try {
            if (logger_)
                logger_->error("[ai] coordinator stopped: {}", e.what());
        } catch (...) {
        }
    } catch (...) {
    }
