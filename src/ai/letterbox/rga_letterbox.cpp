...
public:
    ...
    void import(void* address, int size) {
        if (handle_)
            throw std::logic_error("RGA buffer already imported");
        handle_ = importbuffer_virtualaddr(address, size);
        if (!handle_)
            throw std::runtime_error("RGA virtual buffer import failed");
    }
    ...
    void import_fd(int fd, int size) {
        ...
        if (!handle_)
            throw std::runtime_error("RGA DMA buffer import failed");
    }

    rga_buffer_handle_t get() const noexcept {
        return handle_;
    }
...
RgaLetterbox::RgaLetterbox(int size, int fd, std::size_t bytes, int stride)
    ...
    require_status(imcheckHeader(), "header compatibility");
    impl_->width_stride = stride;
    impl_->output.import_fd(fd, static_cast<int>(bytes));
}
...
const ModelImage& RgaLetterbox::process(const camera::VideoFrame& frame) {
    ...
    if (frame.dma)
        input.import_fd(frame.dma->fd, static_cast<int>(frame.size));
    else
        input.import(const_cast<std::uint8_t*>(frame.data.get()), static_cast<int>(frame.size));
    auto src =
        wrapbuffer_handle_t(input.get(), frame.width, frame.height, frame.stride,
                            frame.dma ? frame.height_stride : frame.height, RK_FORMAT_YCbCr_420_SP);
    auto dst = wrapbuffer_handle_t(s.output.get(), s.size, s.size, s.width_stride, s.size,
                                   RK_FORMAT_RGB_888);
    ...
    require_status(imcheck_t(src, dst, {}, source, destination, {}, IM_SYNC),
