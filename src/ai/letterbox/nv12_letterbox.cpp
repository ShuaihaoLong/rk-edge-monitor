...
LetterboxGeometry nv12_letterbox_geometry(const camera::VideoFrame& f, int size) {
    if(size<=0 || size>4096)throw std::invalid_argument("invalid model image size");
...
void nv12_letterbox(const camera::VideoFrame& f, ModelImage& out, int size) {
    ...
    if(f.dma)mapping=std::make_unique<media::DmaMapping>(f.dma);
    const auto* yplane=mapping?mapping->data():f.data.get();
    const auto* uv=yplane+media::nv12_uv_offset(f);
    auto sample=[](const std::uint8_t* p,int width,int height,float x,float y,int step,int offset,int stride) {
        ...
        auto at=[&](int a,int b){return p[b*stride+a*step+offset];};
    ...
    for(int y=0;y<h;++y) for(int x=0;x<w;++x) {
        ...
        const float l=1.164383f*(sample(yplane,f.width,f.height,sx,sy,1,0,f.stride)-16);
        const float u=sample(uv,f.width/2,f.height/2,sx/2,sy/2,2,0,f.stride)-128;
        const float v=sample(uv,f.width/2,f.height/2,sx/2,sy/2,2,1,f.stride)-128;
