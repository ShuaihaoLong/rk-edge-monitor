...
ResultWriter::ResultWriter(std::string path, std::string event_socket)
    : event_socket_(std::move(event_socket)), path_(std::move(path)) {
    if (event_socket_.size() >= sizeof(sockaddr_un::sun_path))
        throw std::invalid_argument("event socket path too long");
    if (!event_socket_.empty())
        event_fd_ = socket(AF_UNIX, SOCK_DGRAM | SOCK_NONBLOCK | SOCK_CLOEXEC, 0);
    session_ = std::to_string(getpid()) + "-" +
...
void ResultWriter::write(const std::string& status, const DetectionResult* r) {
    ...
    if (!file)
        throw std::runtime_error("cannot write AI result snapshot");
    ...
    message << "{\"session\":" << quote(session_) << ",\"revision\":" << ++revision_
            ...
            << (r ? std::chrono::duration_cast<std::chrono::milliseconds>(
                        r->received_at.time_since_epoch())
                        .count()
                  : 0)
    ...
    file.close();
    if (!file)
        throw std::runtime_error("AI snapshot write failed");
