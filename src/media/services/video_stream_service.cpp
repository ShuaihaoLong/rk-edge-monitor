...
VideoStreamService::VideoStreamService(std::unique_ptr<IVideoPublisher> publisher,
                                       InputProvider input, FaultHandler fault,
                                       std::shared_ptr<spdlog::logger> logger)
    : publisher_(std::move(publisher)), provider_(std::move(input)), fault_(std::move(fault)),
      logger_(std::move(logger)) {
    if (!publisher_ || !provider_)
        throw std::invalid_argument("invalid stream service configuration");
}
...
bool VideoStreamService::start() {
    if (running())
        return true;
    if (worker_.joinable())
        throw std::logic_error("join stream service before restarting");
    stop_ = false;
...
bool VideoStreamService::running() const noexcept {
...
void VideoStreamService::run() noexcept {
    ...
    while (!stop_) {
        try {
            ...
            while (!stop_) {
                ...
                if (!frame) {
                    if (input_->closed())
                        throw std::runtime_error("decoded frame queue closed unexpectedly");
                    continue;
                }
                if (generation != 0 && frame->source_generation != generation) {
                    ...
                    if (logger_)
                        logger_->info("[video_stream] camera session changed; publisher restarted");
                }
            ...
            try {
                if (logger_)
                    logger_->warn("[video_stream] {}; retry in 2s", error.what());
            } catch (...) {
            }
        } catch (...) {
            if (stop_)
    ...
    try {
        if (logger_)
            logger_->info("[video_stream] submitted={}, encoded={}", submitted,
                          publisher_->encoded_frames());
    } catch (...) {
    }
}
}
