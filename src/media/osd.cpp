...
std::string osd_text(const camera::VideoFrame& frame, const std::string& timezone) {
    ...
    if (frame.received_at != std::chrono::system_clock::time_point{}) {
        const auto shifted =
            frame.received_at +
            (timezone == "Asia/Shanghai" ? std::chrono::hours(8) : std::chrono::hours(0));
        const auto seconds = std::chrono::system_clock::to_time_t(shifted);
        std::tm calendar{};
        auto* converted =
            timezone == "local" ? localtime_r(&seconds, &calendar) : gmtime_r(&seconds, &calendar);
        char text[64]{};
        if (converted && std::strftime(text, sizeof(text), "%Y-%m-%d %H:%M:%S", &calendar)) {
            date = text;
            if (timezone == "local") {
                char zone[16]{};
                if (std::strftime(zone, sizeof(zone), "%z", &calendar))
                    date += std::string(" UTC") + zone;
            } else {
...
void draw_osd(std::uint8_t* y, std::size_t y_stride, std::uint8_t* uv, std::size_t uv_stride,
    ...
    if (!y || !uv || width < 2 || height < 2 || width % 2 || height % 2 ||
        y_stride < static_cast<std::size_t>(width) || uv_stride < static_cast<std::size_t>(width))
        return;
    const auto columns = std::min<std::size_t>(64, text.size());
    const int scale =
        std::max(1, std::min({6, height / 240, width / static_cast<int>(columns * 6 + 12)}));
