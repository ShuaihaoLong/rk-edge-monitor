...
void ServiceManager::add(std::unique_ptr<IService> service) {
    if (started_ || attempted_ != 0)
        throw std::logic_error("cannot add a running service");
    if (!service || service->name().empty())
        throw std::invalid_argument("service needs a name");
...
void ServiceManager::stop_all() noexcept {
    ...
    for (std::size_t i = attempted_; i > 0; --i)
        services_[i - 1]->request_stop();
    for (std::size_t i = attempted_; i > 0; --i)
        services_[i - 1]->join();
    attempted_ = 0;
    started_ = false;
