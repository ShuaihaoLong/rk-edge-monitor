...
void append_string(std::string& target, const std::string& value) {
    if (value.size() > 65535)
        throw std::invalid_argument("MQTT string is too long");
...
std::string encode_length(std::size_t length) {
    if (length > 268435455)
        throw std::invalid_argument("MQTT packet is too large");
    std::string bytes;
    do {
...
void send_all(int socket, const char* data, std::size_t size) {
    while (size) {
        ...
        if (sent < 0 && errno == EINTR)
            continue;
        if (sent <= 0)
            throw std::runtime_error("MQTT send failed: " + std::string(std::strerror(errno)));
        data += sent;
...
MqttClient::MqttClient(Config config, std::string client_suffix, std::string status_topic)
    ...
    topic_ = status_topic.empty() ? config_.topic_prefix + "/" + config_.device_id + "/status"
                                  : std::move(status_topic);
}
    ...
    try {
        ...
        while (received < sizeof(reply)) {
            ...
            if (count < 0 && errno == EINTR)
                continue;
            if (count <= 0)
                throw std::runtime_error("MQTT CONNACK timeout");
...
void MqttClient::publish(const std::string& topic, const std::string& payload, bool retained) {
    if (socket_ < 0)
        throw std::logic_error("MQTT client is disconnected");
    std::string body;
...
void MqttClient::subscribe(const std::string& topic) {
    if (socket_ < 0 || subscription_pending_)
        throw std::logic_error("MQTT subscription unavailable");
...
std::vector<MqttClient::Message> MqttClient::receive(int timeout_ms) {
    ...
    if (ready < 0 && errno != EINTR)
        throw std::runtime_error("MQTT receive poll failed");
    if (ready > 0) {
        ...
        if (count > 0)
            receive_buffer_.append(bytes, static_cast<std::size_t>(count));
    }
    ...
    while (receive_buffer_.size() >= 2) {
        ...
        if (length > 4096)
            throw std::runtime_error("MQTT incoming packet exceeds 4096 bytes");
        if (!complete || receive_buffer_.size() < offset + length)
            break;
        ...
        if ((header & 0xf0) == 0x30) {
            ...
            const auto topic_size =
                (static_cast<unsigned char>(body[0]) << 8) | static_cast<unsigned char>(body[1]);
            if (!topic_size || body.size() < 2u + topic_size)
                throw std::runtime_error("invalid MQTT PUBLISH topic");
            messages.push_back(
                {body.substr(2, topic_size), body.substr(2 + topic_size), (header & 1) != 0});
        } else if (header == 0x90) {
