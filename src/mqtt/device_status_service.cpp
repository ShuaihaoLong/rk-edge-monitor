...
std::string local_ipv4() {
    ...
    for (auto* item = interfaces; item; item = item->ifa_next) {
        ...
        if (std::string(item->ifa_name).rfind("eth", 0) == 0 ||
            std::string(item->ifa_name).rfind("en", 0) == 0)
            return address;
        if (fallback.empty())
            fallback = address;
    }
...
bool cpu_times(std::uint64_t& total, std::uint64_t& idle) {
    ...
    std::string name;
    std::uint64_t user = 0, nice = 0, system = 0, idle_value = 0, iowait = 0, irq = 0, softirq = 0,
                  steal = 0;
    if (!(input >> name >> user >> nice >> system >> idle_value >> iowait >> irq >> softirq >>
          steal) ||
        name != "cpu")
        return false;
...
DeviceStatusService::DeviceStatusService(Config config, CameraStatus camera_status,
    ...
    if (config_.device_id.empty() || config_.device_id.size() > 64 ||
        (config_.role != "center" && config_.role != "edge") || config_.publish_interval_ms < 250 ||
        config_.publish_interval_ms > 60000 || config_.keepalive_seconds < 2 ||
        config_.publish_interval_ms >= config_.keepalive_seconds * 1000)
...
bool DeviceStatusService::start() {
    if (running_)
        return true;
    if (worker_.joinable())
        throw std::logic_error("join MQTT service before restarting");
    stop_ = false;
    connected_ = false;
    running_ = true;
...
void DeviceStatusService::request_stop() noexcept {
    stop_ = true;
}
...
std::string DeviceStatusService::status_json(bool online) {
    ...
    if (cpu_times(total, idle) && previous_total_ && total > previous_total_) {
        ...
        usage = 100.0 * static_cast<double>(total_delta - std::min(idle_delta, total_delta)) /
                total_delta;
    }
    ...
    out << "{\"schema\":1,\"device_id\":" << json_string(config_.device_id)
        ...
        << ",\"online\":" << (online ? "true" : "false") << ",\"camera_online\":"
        << (online && camera_status_ && camera_status_() ? "true" : "false")
    ...
    if (temperature)
        out << std::fixed << std::setprecision(1) << *temperature;
    else
        out << "null";
    ...
    while (!stop_) {
        try {
            ...
            if (!client_.connected() && now >= retry_at) {
                ...
                try {
                    if (logger_)
                        logger_->info("[mqtt] connected, topic={}", client_.topic());
                } catch (...) {
                }
            }
            ...
            try {
                if (logger_)
                    logger_->warn("[mqtt] {}; retry in 2s", error.what());
            } catch (...) {
            }
        }
