...
speed_t baud(unsigned value) {
    ...
    case 230400:
        return B230400;
    default:
        throw std::invalid_argument("unsupported STM32 baud rate");
    }
}
}
...
std::size_t SerialPort::read(std::uint8_t* data, std::size_t capacity, int timeout_ms) {
    if (fd_ < 0)
        throw std::logic_error("STM32 serial disconnected");
...
void SerialPort::write(const std::uint8_t* data, std::size_t size, int timeout_ms) {
    ...
    while (size) {
        const auto remaining = std::chrono::duration_cast<std::chrono::milliseconds>(
                                   ...
                                   .count();
        if (remaining <= 0)
            throw std::runtime_error("STM32 serial write timeout; command outcome unknown");
        ...
        if (descriptor.revents & (POLLERR | POLLHUP | POLLNVAL))
            throw std::runtime_error(
                "STM32 serial disconnected during write; command outcome unknown");
