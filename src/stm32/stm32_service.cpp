...
std::optional<Dht11Sample> decode_dht11(const Frame& frame, std::int64_t received_at_ms) {
    if (frame.type != msg_dht11_data || frame.payload.size() != 4)
        return std::nullopt;
    const auto& p = frame.payload;
    if (p[0] > 50 || p[1] > 9 || p[2] > 100 || p[3] > 9 || (p[0] == 50 && p[1]) ||
        (p[2] == 100 && p[3]))
        return std::nullopt;
...
bool Stm32Service::start() {
    if (running_)
        return true;
    if (serial_thread_.joinable() || mqtt_thread_.joinable())
        throw std::logic_error("join STM32 service before restart");
    {
...
void Stm32Service::request_stop() noexcept {
    stop_ = true;
}
...
bool Stm32Service::online_locked() const {
    return serial_connected_ && sample_ &&
           Clock::now() - last_sample_ < std::chrono::milliseconds(config_.stale_timeout_ms);
}

core::HealthSnapshot Stm32Service::health() const {
    if (!running_)
        return {core::ServiceState::stopped, {}};
    std::lock_guard lock(mutex_);
    if (!online_locked())
        return {core::ServiceState::degraded, "STM32 data unavailable or stale"};
    if (!mqtt_connected_)
        return {core::ServiceState::degraded, "STM32 MQTT disconnected"};
    return {core::ServiceState::running, {}};
}

std::string Stm32Service::status_json(bool shutting_down) const {
    std::lock_guard lock(mutex_);
    return std::string("{\"schema\":1,\"online\":") +
           (!shutting_down && online_locked() ? "true" : "false") +
...
void Stm32Service::warn(const char* component, const std::exception& error) const noexcept {
    try {
        if (logger_)
            logger_->warn("[stm32/{}] {}", component, error.what());
    } catch (...) {
    }
}
...
void Stm32Service::result(std::uint32_t sequence, const char* state) {
    event(prefix_ + "/command_result",
          "{\"schema\":1,\"sequence\":" + std::to_string(sequence) + ",\"state\":\"" + state +
              "\",\"updated_at_ms\":" + std::to_string(unix_ms()) + '}');
}
...
void Stm32Service::enqueue_command(const std::string& payload, bool retained) {
    ...
    if (retained) {
        result(sequence, "rejected_retained");
        return;
    }
    if (payload.size() > max_payload) {
        result(sequence, "rejected_length");
        return;
    }
    if (!online_locked()) {
        result(sequence, "rejected_offline");
        return;
    }
    if (commands_.size() >= 16) {
        result(sequence, "rejected_busy");
        return;
    }
...
void Stm32Service::record_frame(const Frame& frame) {
    ...
    if (auto sample = decode_dht11(frame, unix_ms())) {
        ...
        event(prefix_ + "/ack", "{\"schema\":1,\"sequence\":" + std::to_string(frame.sequence) +
                                    ",\"payload_hex\":\"" + hex(frame.payload) +
                                    "\",\"received_at_ms\":" + std::to_string(unix_ms()) + '}');
    }
}
...
void Stm32Service::serial_loop() noexcept {
    ...
    while (!stop_) {
        try {
            if (!port.connected()) {
                port.open(config_.device, config_.baud_rate);
                parser.reset();
                std::lock_guard lock(mutex_);
            ...
            for (const auto& frame : parser.feed(bytes, count))
                record_frame(frame);
            std::optional<Frame> command;
            {
                ...
                if (command && !online_locked()) {
                    result(command->sequence, "rejected_offline");
                    command.reset();
                }
            }
            if (command) {
    ...
    while (!stop_) {
        try {
            ...
            for (const auto& message : client.receive(20))
                if (message.topic == prefix_ + "/command")
                    enqueue_command(message.payload, message.retained);
            ...
            if (outgoing) {
                ...
                std::lock_guard lock(mutex_);
                if (!events_.empty() && events_.front().id == outgoing->id)
                    events_.pop_front();
            }
        } catch (const std::exception& error) {
            mqtt_connected_ = false;
            client.disconnect(status_json(true));
            {
                std::lock_guard lock(mutex_);
                for (const auto& command : commands_)
                    result(command.sequence, "rejected_disconnected");
                commands_.clear();
            }
            warn("mqtt", error);
            pause(stop_, config_.reconnect_interval_ms);
