...
std::uint32_t read_le(const std::uint8_t* bytes, unsigned count) {
    ...
    for (unsigned i = 0; i < count; ++i) result |= std::uint32_t(bytes[i]) << (i * 8);
    return result;
}
...
std::vector<std::uint8_t> encode(const Frame& frame) {
    if (frame.payload.size() > max_payload) throw std::invalid_argument("STM32 payload exceeds 128 bytes");
...
FrameParser::FrameParser(std::chrono::milliseconds timeout) : timeout_(timeout) {
    if (timeout.count() <= 0) throw std::invalid_argument("STM32 frame timeout must be positive");
    buffer_.reserve(14 + max_payload);
}

void FrameParser::reset() noexcept { buffer_.clear(); candidate_ = false; }

std::vector<Frame> FrameParser::feed(const std::uint8_t* data, std::size_t size, Clock::time_point now) {
...
void FrameParser::parse(std::vector<Frame>& frames, Clock::time_point now) {
    while (!buffer_.empty()) {
        ...
        if (!candidate_) { candidate_ = true; candidate_since_ = now; }
        ...
        frames.push_back({buffer_[3], read_le(buffer_.data() + 4, 4),
