...
void flush(lv_display_t* display, const lv_area_t* area, std::uint8_t* data) {
    ...
    for (int y = area->y1; y <= area->y2; ++y)
        std::memcpy(s.pixels.data() + y * 1024 + area->x1, data + (y - area->y1) * width * 4,
                    width * 4);
    if (lv_display_flush_is_last(display)) {
        if (SDL_UpdateTexture(s.texture, nullptr, s.pixels.data(), 1024 * 4) ||
            SDL_RenderClear(s.renderer) ||
            SDL_RenderCopy(s.renderer, s.texture, nullptr, &s.viewport))
            s.failed = true;
...
void update_weather(const std::string& path, lv_obj_t* place, lv_obj_t* weather, lv_obj_t* age) {
    ...
    if (!input || input.tellg() > 8192) {
        lv_label_set_text(place, "天气 · 合肥");
        lv_label_set_text(weather, "等待联网");
        lv_label_set_text(age, "暂无天气数据");
        return;
    }
    input.seekg(0);
    std::string text((std::istreambuf_iterator<char>(input)), {});
    std::unique_ptr<json_object, decltype(&json_object_put)> obj(json_tokener_parse(text.c_str()),
                                                                 json_object_put);
    if (!obj || !json_object_is_type(obj.get(), json_type_object)) {
        lv_label_set_text(weather, "天气数据不可用");
        return;
    }
    ...
    if (!updated || !std::isfinite(temp)) {
        lv_label_set_text(weather, "天气暂不可用");
        lv_label_set_text(age, "联网后自动更新");
        return;
    }
    ...
    lv_label_set_text_fmt(age, "%s · %lld分钟前更新", cached ? "离线/缓存" : "已更新",
                          std::max(0LL, elapsed / 60));
}
...
int main(int argc, char** argv) {
    try {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (arg == "--help") {
                std::cout
                    << "rkmon-display [--config FILE] [--settings FILE] [--seconds N] [--capture FILE.bmp]\n"
                       "Local monitor; Escape exits. --seconds 0 runs continuously.\n";
                return 0;
            }
            if ((arg == "--seconds" || arg == "--capture" || arg == "--config" ||
                 arg == "--settings") &&
                i + 1 < argc) {
                if (arg == "--seconds")
        ...
        if (seconds < 0 || seconds > 86400)
            throw std::runtime_error("seconds must be 0..86400");
        const auto runtime = rkmon::app::load_config(config);
        if (!runtime.stream)
            throw std::runtime_error("local display requires configured RTSP stream");
        std::unique_ptr<json_object, decltype(&json_object_put)> settings_json(
            json_object_from_file(settings.c_str()), json_object_put);
        ...
        auto setting = [&](const char* key) {
            ...
            if (!json_object_object_get_ex(settings_json.get(), key, &value) ||
        ...
        if (timezone.empty() || timezone.front() == '/' ||
            ...
            !std::ifstream("/usr/share/zoneinfo/" + timezone))
        ...
        std::cout << "SDL window=" << window_width << "x" << window_height << " flags=0x"
                  << std::hex << SDL_GetWindowFlags(s.window) << std::dec << std::endl;
        ...
        std::cout << "SDL output=" << output_width << "x" << output_height
                  << " viewport=" << s.viewport.x << "," << s.viewport.y << " " << s.viewport.w
                  << "x" << s.viewport.h << std::endl;
        SDL_RenderSetLogicalSize(s.renderer, 1024, 600);
        SDL_RendererInfo ri{};
        SDL_GetRendererInfo(s.renderer, &ri);
        std::cout << "SDL backend=" << SDL_GetCurrentVideoDriver() << " renderer=" << ri.name
                  << std::endl;
        s.texture = SDL_CreateTexture(s.renderer, SDL_PIXELFORMAT_ARGB8888,
                                      SDL_TEXTUREACCESS_STREAMING, 1024, 600);
        if (!s.texture)
            throw std::runtime_error(SDL_GetError());
        ...
        lv_display_set_buffers(display, draw.data(), nullptr, draw.size() * 4,
                               LV_DISPLAY_RENDER_MODE_PARTIAL);
        lv_display_set_user_data(display, &s);
        lv_display_set_flush_cb(display, flush);
        lv_obj_set_style_bg_color(lv_screen_active(), lv_color_hex(0x2a211c), 0);
        auto* font = lv_freetype_font_create(font_path.c_str(), LV_FREETYPE_FONT_RENDER_MODE_BITMAP,
                                             20, LV_FREETYPE_FONT_STYLE_NORMAL);
        if (!font)
            throw std::runtime_error("cannot load CJK font");
        auto* compact_font =
            lv_freetype_font_create(font_path.c_str(), LV_FREETYPE_FONT_RENDER_MODE_BITMAP, 16,
                                    LV_FREETYPE_FONT_STYLE_NORMAL);
        if (!compact_font)
            throw std::runtime_error("cannot load compact CJK font");
        ...
        lv_obj_set_width(place, 220);
        lv_label_set_long_mode(place, LV_LABEL_LONG_DOT);
        lv_obj_set_width(weather, 220);
        lv_obj_set_width(weather_age, 220);
        ...
        if (runtime.mqtt && runtime.stm32)
            telemetry = std::make_unique<rkmon::display::Telemetry>(
                *runtime.mqtt, runtime.stm32->stale_timeout_ms);
        ...
        while (!stopping && (!seconds || Clock::now() - start < std::chrono::seconds(seconds))) {
            ...
            if (has_video) {
                ...
                if (seconds)
                    render_ms.push_back(
                        std::chrono::duration<double, std::milli>(now - began).count());
                if (!frames)
                    first = now;
                else {
                    double gap = std::chrono::duration<double, std::milli>(now - last).count();
                    if (seconds)
                        gaps_ms.push_back(gap);
                    if (gap > 100)
                        ++stalls;
                }
                last = now;
            ...
            if (now >= next_clock) {
                ...
                if (source_ad)
                    ...
                    lv_label_set_text(stats, video_online ? "实时画面  ·  本机视频流"
                                                          : reader.status().c_str());
                if (telemetry) {
                    ...
                    if (sensor.online) {
                        ...
                        lv_label_set_text(temperature, "温度  --");
                        lv_label_set_text(humidity, "湿度  --");
                    }
                }
        ...
        if (!capture.empty()) {
            auto* surface = SDL_CreateRGBSurfaceWithFormatFrom(s.pixels.data(), 1024, 600, 32, 4096,
        ...
        const double cpu = ru.ru_utime.tv_sec + ru.ru_utime.tv_usec / 1e6 + ru.ru_stime.tv_sec +
                           ru.ru_stime.tv_usec / 1e6;
        std::cout << "RESULT frames=" << frames << " seconds=" << elapsed << " fps="
                  << (frames > 1
                          ? (frames - 1) / std::chrono::duration<double>(last - first).count()
                          : 0)
                  << " cpu_percent=" << cpu / elapsed * 100 << " peak_rss_kib=" << ru.ru_maxrss
                  << " render_p50_ms=" << percentile(render_ms, .5)
                  << " render_p95_ms=" << percentile(render_ms, .95)
