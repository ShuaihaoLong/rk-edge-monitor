...
def configuration(path):
    ...
    result = dict(
        ...
        socket=c['event_socket'],
        host=c.get('host', '127.0.0.1'),
        port=c.getint('port', 9010),
...
class Store:
    ...
    def inspect(self, path, completed=False):
        ...
        with self.lock, self.connect() as db:
            db.execute(
                '''INSERT INTO recordings(path,channel,start_ms,end_ms,duration,bytes,mtime_ns,status,updated_ms)
                ...
                (
                    ...
                    'camera',
                    start,
                    start + round(duration * 1000),
                    duration,
                    after.st_size,
                    after.st_mtime_ns,
                    'ready' if completed else 'recovered',
                    int(time.time() * 1000),
                ),
            )
    ...
    def receive(self, message):
        ...
        if kind in ('segment_open', 'segment_complete'):
            ...
            if kind == 'segment_complete':
                ...
                with self.lock, self.connect() as db:
                    db.execute(
                        '''INSERT INTO recordings(path,channel,start_ms,status,updated_ms)
                        VALUES(?,?,?,'writing',?) ON CONFLICT(path) DO UPDATE SET status='writing' ''',
                        (
                            str(path.relative_to(self.root)),
                            'camera',
                            self.file_start(path),
                            int(time.time() * 1000),
                        ),
                    )
            return
    ...
    def save_event(self, event):
        with self.lock, self.connect() as db:
            if event['id'] is None:
                cursor = db.execute(
                    '''INSERT INTO events(session,generation,class_id,label,start_ms,end_ms,confidence,sequence)
                    VALUES(:session,:generation,:class_id,:label,:start_ms,:end_ms,:confidence,:sequence)''',
                    event,
                )
                event['id'] = cursor.lastrowid
            else:
                db.execute(
                    'UPDATE events SET end_ms=:end_ms,confidence=:confidence,sequence=:sequence WHERE id=:id',
                    event,
                )
    ...
    def reconcile(self):
        ...
        for path in sorted((self.root / 'camera').glob('*/*.mp4')):
            ...
            try:
                ...
                if now - stat.st_mtime < 90 or (
                    old
                    and old['mtime_ns'] == stat.st_mtime_ns
                    and old['status'] in ('ready', 'recovered', 'invalid')
                ):
                    continue
                if count >= 16:
                    continue
                ...
                try:
                    ...
                    with self.lock, self.connect() as db:
                        db.execute(
                            '''INSERT INTO recordings(path,channel,start_ms,bytes,mtime_ns,status,updated_ms)
    ...
    def listing(self, day):
        ...
        with self.connect() as db:
            rows = [
                dict(r)
                for r in db.execute(
                    '''SELECT id,start_ms,end_ms,duration,bytes,status FROM recordings
            ...
            events = [
                dict(r)
                for r in db.execute(
                    '''SELECT id,class_id,label,start_ms,end_ms,confidence FROM events
                WHERE start_ms<? AND end_ms>=? ORDER BY start_ms LIMIT 2000''',
                    (end, start),
                )
            ]
        return dict(
    ...
    def playback(self, stamp, duration):
        ...
        with self.connect() as db:
            row = db.execute(
                '''SELECT * FROM recordings WHERE channel='camera' AND status IN ('ready','recovered')
                AND start_ms<=? AND end_ms>? ORDER BY start_ms DESC LIMIT 1''',
                (stamp, stamp),
            ).fetchone()
        ...
        query = urlencode(
            dict(
                path='camera',
                start=dt.datetime.fromtimestamp(stamp / 1000, UTC).isoformat(),
        ...
        return dict(
            url='/recording-media/get?' + query,
            media_url=f'/api/recordings/{row["id"]}/media#t={offset:.3f}',
            start_ms=stamp,
            end_ms=stamp + round(length * 1000),
            recording_id=row['id'],
        )
...
class AdStore:
    ...
    def _write_playlist(self):
        with self.lock, self.connect() as db:
            rows = db.execute(
                'SELECT filename FROM ads WHERE enabled=1 ORDER BY sort_order,id'
            ).fetchall()
        self._write_text(
            self.playlist_path, json.dumps([row['filename'] for row in rows], ensure_ascii=False)
        )
        ...
        try:
            ...
            with self.lock, self.connect() as db:
                sort_order = db.execute(
                    'SELECT COALESCE(MAX(sort_order), -1)+1 FROM ads'
                ).fetchone()[0]
                db.execute(
    ...
    def reorder(self, identifiers):
        if not isinstance(identifiers, list) or len({str(value) for value in identifiers}) != len(
            identifiers
        ):
    ...
    def do_GET(self):
        try:
            ...
            if parsed.path == '/api/recordings/status':
                ...
                result = dict(
                    status='degraded' if store.error else 'ok',
                    detail=store.error,
                    root=str(store.root),
                ...
                with store.connect() as db:
                    ranges = db.execute(
                        "SELECT start_ms,end_ms FROM recordings WHERE status IN ('ready','recovered')"
                    ).fetchall()
                days = set()
                ...
                self.send_header('Content-Type', 'video/mp4')
                self.send_header(
                    'Content-Disposition', f'attachment; filename="recording-{identifier}.mp4"'
                )
                self.send_header('X-Accel-Redirect', '/_recordings/' + quote(row['path'], safe='/'))
                self.end_headers()
                return
